...
database = mysql.connector.connect(host="localhost", user="root", password="Sy070076#", database="cadoctor")
...
class LoginRegister:
        ...
        def login():
            ...
            if l_email_entry.cget('fg') == '#333333' and l_password_entry.cget('fg') == '#333333':
                ...
                if user_email.endswith('@gmail.com'):
                    cursor.execute('''SELECT user_id, user_type, user_password, email_verified FROM user WHERE user_email=%s''', (user_email,))
                    user_data = cursor.fetchone()
                    # If able to fetch means the user account is existed
                    if user_data:
                        stored_password_hash = user_data[2]
                        email_verified = user_data[3] if len(user_data) > 3 else True  # Default to True for backward compatibility
                        
                        # Check if email is verified
                        if not email_verified:
                            l_validate_login_label.config(text='Email not verified. Please check your email and verify your account.')
                            return
                        
                        # Verify password using bcrypt
                        if verify_password(user_password, stored_password_hash):
                            user_id = user_data[0]
            ...
            if fp_email_entry.cget('fg') == '#333333'\
                ...
                if user_email.endswith('@gmail.com'):
                    ...
                    if user_id:
                        ...
                        if is_valid:
                            if new_password == fp_confirmed_entry.get():
                                fp_validate_update_label.config(text='')
                                # Hash password before storing
                                hashed_password = hash_password(new_password)
                                cursor.execute('''UPDATE user SET user_password=%s WHERE user_id=%s''', (hashed_password, user_id[0]))
                                database.commit()
                            ...
                            fp_validate_update_label.config(text=error_msg)
                    else:
            ...
            if ru_name_entry.cget('fg') == '#333333' and ru_ic_passport_entry.cget('fg') == '#333333' \
                ...
                if user_email.endswith('@gmail.com'):
                    ...
                    if not existing_email:
                        password = ru_password_entry.get()
                        # Validate password strength
                        is_valid, error_msg = validate_password_strength(password)
                        if is_valid:
                            if password == ru_confirmed_entry.get():
                                ru_validate_register_label.config(text='')
                                # Hash password before storing
                                hashed_password = hash_password(password)
                                
                                # Email verification is REQUIRED - Generate code and send email
                                verification_code = generate_verification_code(6)
                                code_expires = datetime.now() + timedelta(minutes=10)
                                
                                # Send verification email
                                if not send_verification_email_html(user_email, verification_code):
                                    ru_validate_register_label.config(text='Failed to send verification email. Please try again.')
                                    return
                                
                                # Insert user with email verification (not verified yet)
                                cursor.execute('''INSERT INTO user (user_email, user_password, user_type, 
                                                   email_verified, verification_code, verification_code_expires) 
                                                   VALUES (%s, %s, %s, %s, %s, %s)''',
                                               (user_email, hashed_password, 'user', False, 
                                ...
                                if user_id:
                                    ...
                                    if not self.show_email_verification_popup(user_email, verification_code, user_id[0]):
                                        ru_validate_register_label.config(text='Email verification is required. Please check your email.')
                                        return
                                    ...
                                    database.commit()
                                    messagebox.showinfo('Success', 'Register User Account Successfully with Email Verified')
                                    # Directed the user to the login page
                                    self.show_login()
                            else:
                                ru_validate_register_label.config(text='Password does not match')
                        else:
                            ru_validate_register_label.config(text=error_msg)
                    else:
    ...
    def show_email_verification_popup(self, email, verification_code, user_id):
        """
        ...
        def verify_code():
            ...
            if len(code) != 6 or not code.isdigit():
                error_label.config(text='Please enter a valid 6-digit code')
                return
            
            # Check if code matches and hasn't expired
            cursor.execute('''SELECT verification_code, verification_code_expires FROM user WHERE user_id=%s''', (user_id,))
            db_data = cursor.fetchone()
            
            if not db_data or not db_data[0]:
                error_label.config(text='Verification code not found. Please register again.')
                return
            ...
            if expires and datetime.now() > expires:
                error_label.config(text='Verification code has expired. Please register again.')
                return
            
            # Verify code
            if code == db_code:
                # Mark email as verified and clear verification code
                cursor.execute('''UPDATE user SET email_verified=TRUE, verification_code=NULL, 
                                 verification_code_expires=NULL WHERE user_id=%s''', (user_id,))
                database.commit()
                result['verified'] = True
                popup.destroy()
            else:
                error_label.config(text='Invalid code. Please try again.')
        ...
        def resend_code():
            ...
            if send_verification_email_html(email, new_code):
                # Update database with new code
                cursor.execute('''UPDATE user SET verification_code=%s, verification_code_expires=%s 
                                 WHERE user_id=%s''', (new_code, new_expires, user_id))
                database.commit()
                error_label.config(text='New verification code sent! Please check your email.', fg='green')
                code_entry.delete(0, tk.END)
                code_entry.focus_set()
            else:
                error_label.config(text='Failed to resend code. Please try again later.')
        
        def cancel():
            # Email verification is required
            if messagebox.askyesno('Email Verification Required', 
        ...
        def show_reset():
            ...
            def reset():
                ...
                if old_entry.cget('fg') == '#333333' and new_entry.cget('fg') == '#333333' and confirm_entry.cget('fg') == '#333333':
                    ...
                    previous_password_hash = cursor.fetchone()[0]
                    # Verify old password using bcrypt
                    if verify_password(old_entry.get(), previous_password_hash):
                        ...
                        if is_valid:
                            if new_password == confirm_entry.get():
                                # Hash password before storing
                                hashed_password = hash_password(new_password)
                                cursor.execute('''UPDATE user SET user_password=%s WHERE user_id=%s''', (hashed_password, self.user_id))
                                database.commit()
                            ...
                            save_error_label.config(text=error_msg)
                    else:
