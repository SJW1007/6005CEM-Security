    ...
    def show_registering_user(self):
        ...
        def register_user():
            ...
            if ru_name_entry.cget('fg') == '#333333' and ru_ic_passport_entry.cget('fg') == '#333333' \
                ...
                if not self.validate_phone_number(ru_contact_entry.get()):
                    ...
                    if not existing_email:
                        if len(ru_password_entry.get()) >= 8:
                            if ru_password_entry.get() == ru_confirmed_entry.get():
                                ...
                                if user_id:
                                    # Sanitize inputs
                                    patient_name = self.sanitize_input(ru_name_entry.get(), max_length=100)
                                    patient_ic = self.sanitize_input(ru_ic_passport_entry.get().replace('-', '').replace(' ', ''), max_length=50)
                                    patient_gender = self.sanitize_input(ru_gender_entry.cget('text'), max_length=10)
                                    patient_address = self.sanitize_text_widget(ru_address_entry.get('1.0', 'end'), max_length=500)
                                    patient_contact = self.sanitize_phone_number(ru_contact_entry.get())
                                    ...
                                    cursor.execute('''INSERT INTO patient (patient_name, patient_ic_passport, patient_gender, 
                                                   ...
                                                   (patient_name, patient_ic, patient_gender, patient_address, patient_contact, user_id[0]))
                                    database.commit()
    ...
    def show_registering_clinic(self):
        ...
        def register_clinic():
            ...
            if rc_name_entry.cget('fg') == '#333333' and rc_operation_entry.cget('fg') == '#333333' \
                ...
                if img and img.lower().endswith(('.jpg', '.jpeg', '.png')):
                            ...
                            if len(rc_password_entry.get()) >= 8:
                                if rc_password_entry.get() == rc_confirmed_entry.get():
                                    ...
                                    if user_id:
                                        ...
                                        clinic_address = self.sanitize_text_widget(rc_address_entry.get('1.0', 'end'), max_length=500)
                                        clinic_description = self.sanitize_text_widget(rc_describe_entry.get('1.0', 'end'), max_length=2000)
                                        ...
                                        cursor.execute('''INSERT INTO clinic (clinic_name, clinic_operation, clinic_address, 
                                                       ...
                                                       (clinic_name, clinic_operation, clinic_address, clinic_description, 
                                                           clinic_contact, img_binary_data, 0, user_id[0]))
                                        database.commit()
